...
$auth=FAILED;
if($action=="login"){
	$sql="SELECT * FROM user_operator WHERE id='$id' AND user_key='$password'";
	if($db->query($sql)){
		if($db->fetchRow($sql)!=null){
			$newPassword=generatePassword();
			$sql="UPDATE user_operator
...
if($auth==SUCCESS && isset($action)){	//jika $action tidak null
	switch($action) {
		case "getParkir":	
			$sql="SELECT p.id, p.name, p.address, p.available, p.capacity 
					FROM `parkir` p 
					RIGHT JOIN `parkir_operator` op
					ON p.id = op.id_parkir
					WHERE op.operator='$id'
					;";
		...
		case "getDetailParkir":	
			...
			$sql="SELECT id,latitude,longitude,name,address,price,capacity,available
					...
					WHERE id='$parkirId'
					;";
			$out=query($db,$sql);		//gunakan method get untuk ambitl data! $out= hasil return method get tadi
			//Contoh Hasil: [{"nama":"albert"}]
		break;
...
function auth($db,$id,$password){
	$sql="SELECT * FROM user_operator WHERE id='$id' AND password='$password'";
	if($db->query($sql)){
		if($db->fetchRow($sql)!=null){
			return SUCCESS;
		}
